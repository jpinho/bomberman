...
public class LevelFileParser {
	...
	public static void loadLevelFromFile(Resources res, String levelFile) {
		...
		try {
			...
			throw new InternalError("Can't read level input file: "
					+ e.getMessage());
		}
	}
	...
	public static void loadLevelFromString(String levelRep) {
		...
		try {
			...
			throw new InternalError(
					"Unable to read level representation string.");
		}
	}

	private static void buildLevel(BufferedReader reader, boolean isServer)
			throws IOException {
		int rows, cols;
		int max_players = 0;
		String line;
		GameLevel level = GameLevel.getInstance();
		for (line = reader.readLine(); !line.matches("MAP.*"); line = reader
				.readLine()) {
			if (line.matches("LN.*")) {
				level.setLevel_name(line.substring(3, line.length()));
			} else if (line.matches("GD.*")) {
				level.setTimeLeft(Integer.parseInt(line.substring(3,
						line.length())));
			} else if (line.matches("ET.*")) {
				level.setExplosion_timeout(Float.parseFloat(line.substring(3,
						line.length())));
			} else if (line.matches("ER.*")) {
				level.setExplosion_range(Integer.parseInt(line.substring(3,
						line.length())));
			} else if (line.matches("ED.*")) {
				level.setExplosion_duration(Integer.parseInt(line.substring(3,
						line.length())));
			} else if (line.matches("RS.*")) {
				level.setEnemy_speed(Float.parseFloat(line.substring(3,
						line.length())));
			} else if (line.matches("PR.*")) {
				level.setRobot_score(Integer.parseInt(line.substring(3,
						line.length())));
			} else if (line.matches("PO.*")) {
				level.setOpponent_score(Integer.parseInt(line.substring(3,
						line.length())));
			} else if (line.matches("MP.*")) {
				max_players = Integer
						.parseInt(line.substring(3, line.length()));
			}
		}
		int i;
	...
	private static GameBoard process_map(int rows, int cols,
			BufferedReader reader, int max_players, boolean isServer)
			throws IOException {
		GameBoard board;
		...
		int row = 0;
		for (String map_entry = reader.readLine(); map_entry != null; map_entry = reader
				.readLine(), row++) {
	...
	private static ArrayList<BFDescriptor> process_map_line(String line,
			int row, GameBoard board) {
		int p;
		ArrayList<BFDescriptor> bombAndFireToShow = new ArrayList<BFDescriptor>();
		for (int col = 0, y = 0; col < line.length(); col++, y++) {
			...
			if (c == '-') {
				...
				Log.d("LevelFileParser", "Added player " + p + " at (" + row
						+ "," + y + ")");
			}
