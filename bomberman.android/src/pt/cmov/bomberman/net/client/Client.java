...
public class Client {
	...
	public void parse_msg(String[] tokens) {
		...
		if (tokens[0].equalsIgnoreCase("board")) {
			...
			Client.getInstance().sendUpdatePlayerName(Integer.parseInt(tokens[1]),  GameLevel.getInstance().getPlayer_name());
			...
			GameLevel.getInstance().getBoard()
					.actionMovePlayer(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[2]));
		}
			...
			Player p = ((GameBoardClient) GameLevel.getInstance().getBoard()).findPlayer(pid);
			
			if(p!=null) p.setPlayerName(name);
		}
