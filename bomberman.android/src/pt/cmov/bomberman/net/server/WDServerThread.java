		...
		while (!Thread.currentThread().isInterrupted()) {
			try {
				...
				if ((new_player_id = GameLevel.getInstance().isJoinable()) != -1) {
					Log.d("ServerHost", "Accepted new player, id = "
							+ new_player_id);
					WDCommunicationThread commThread = new WDCommunicationThread(
							clientSocket, new_player_id);
					new Thread(commThread).start();
					Player newPlayer = GameLevel.getInstance().getBoard()
							.findPlayer(new_player_id);
					Server.getInstance().broadcastPlayerJoined(new_player_id,
							newPlayer.getX(), newPlayer.getY());
				} else {
					/* Level is full */
					Log.d("ServerHost",
							"Declined new request - no room for more players!");
					clientSocket.close();
				}
			} catch (IOException e) {
				// Ignore this client.
