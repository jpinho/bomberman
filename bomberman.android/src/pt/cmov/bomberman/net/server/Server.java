...
public class Server {
	...
	public String parse_msg(String[] tokens) {
		...
		if (command.equalsIgnoreCase("move"))
			board.actionMovePlayer(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[2]));
			...
			broadcastMsg("gpname_response " + Integer.parseInt(tokens[1]) + " " + tokens[2] + "\n");

			synchronized (players) {
				for (RemotePlayer rp : players) {
					if (rp.getPlayer_id() == Integer.parseInt(tokens[1])) {

						for (Player p : GameLevel.getInstance().getBoard().getPlayers()) {
							if (p.getPlayer_number() != Integer.parseInt(tokens[1])){
								rp.sendMsg("gpname_response " + p.getPlayer_number() + " " + p.getPlayerName() + "\n");
							}
						}
			...
			if (localPlayer != null) {
				String name = localPlayer.getPlayerName() != null ? localPlayer.getPlayerName()
						.replace(" ", "_") : "NULL";
	...
	public void sendClientReply(RemotePlayer player, String command, String result) {
