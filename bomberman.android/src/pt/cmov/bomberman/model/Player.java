...
public class Player extends GameObject {
	...
	public Player(int player, int x, int y) {
		...
		plantedBomb = false;
		score = 0;
		label = new PlayerLabelView(String.format("%s-%d", DEFAULT_NAME, player_number));
	}
	...
	public int notifyExplosion(Player responsible) {
		GameLevel.getInstance().getBoard()
				.kill(getPlayer_number(), "Player " + responsible.getPlayer_number()); // TODO
																						// Use
																						// player
																						// name
																						// instead
		Server.getInstance().broadcastPlayersKilled(
				"die Player" + responsible.getPlayer_number() + " " + getPlayer_number() + "\n");
