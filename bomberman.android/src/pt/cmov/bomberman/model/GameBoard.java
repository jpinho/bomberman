...
public class GameBoard {
	/* horizontalExcess and verticalExcess are the top and left offsets
	 * of the map location in pixels.
	 */
	...
	private IGameObject[][] board; // Stores game objects in world coordinates
	...
	/* This is called by the level parser routine when it is reading a configuration file and finds
	 * the map's dimensions.
	 * This function will decide how to scale an object such that a map with nRows rows and nCols columns
	 * fits in an XxY screen. To do so, it computes the dimensions (in pixels) that each object will have, and then
	 * it calculates how much space is empty. Empty space is filled with a border image (rocks) 
	 */
	public void setScreenDimensions(int width, int height) {
		int max_object_width = (int) Math.floor((width*1.0)/nCols);
		int max_object_height = (int) Math.floor((height*1.0)/nRows);
		
		/* delta is the amount (in pixels) of increment or decrement on each side of the object.
		 * If the object fits into the space defined by [max_object_width,max_object_height], then delta
		 * is positive, because we can zoom the object until one of its sides hits the limit.
		 * Otherwise, delta is negative, because we must resize the object until all of its sides fit into
		 * the space.
		 * The use of delta ensures that the aspect ratio of the image file is kept.
		 */
		int delta;
	...
	// Draws borders on left and right sides of the screen that are outside of the map zone.
