...
public class GameArenaActivity extends Activity {
	...
	/*
	 * God mode: - players can plant unlimited number of bombs at the same time
	...
	private void setupGameStatusUpdate() {
		final GameStatusViewProxy gameStatusProxy = new GameStatusViewProxy(
				this.findViewById(R.id.gameTitleContainer));
	...
	public boolean onKeyUp(int keyCode, android.view.KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_W) {
			GameLevel.getInstance().getBoard()
					.actionMovePlayer(1, JoystickView.FRONT);
			return true;
		} else if (keyCode == KeyEvent.KEYCODE_S) {
			GameLevel.getInstance().getBoard()
					.actionMovePlayer(1, JoystickView.BOTTOM);
			return true;
		} else if (keyCode == KeyEvent.KEYCODE_A) {
			GameLevel.getInstance().getBoard()
					.actionMovePlayer(1, JoystickView.LEFT);
			return true;
		} else if (keyCode == KeyEvent.KEYCODE_D) {
			GameLevel.getInstance().getBoard()
					.actionMovePlayer(1, JoystickView.RIGHT);
			return true;
	...
	protected void onPause() {
		super.onPause();
		synchronized (gameView.getThread().getmPauseLock()) {
			gameView.getThread().setmPaused(true);
		}
	...
	protected void onResume() {
		super.onResume();
		synchronized (gameView.getThread().getmPauseLock()) {
			gameView.getThread().setmPaused(false);
			gameView.getThread().getmPauseLock().notifyAll();
		}
	}
		...
		jsv.setOnJoystickMoveListener(new OnJoystickMoveListener() {
			...
			public void onValueChanged(int angle, int power, int direction) {
				if (direction == JoystickView.BOTTOM
						|| direction == JoystickView.FRONT
						|| direction == JoystickView.LEFT
						|| direction == JoystickView.RIGHT) {

					GameLevel.getInstance().getBoard()
							.actionMovePlayer(direction);
				}
			}
		});
	}
	...
	private void setupSounds() {
		try {
			// not emulator
			if (!(android.os.Build.MODEL.equals("google_sdk") || android.os.Build.MODEL
					.equals("sdk"))) {
