...
public class MainGamePanel extends SurfaceView implements
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		/*
		 * Upon surface creation, we must call loadLevel() to load the bootstrap
		 * level. The parser will read the level file and build the level
		 * accordingly; level attributes are stored in currentGameLevel, and the
		 * map layout is retrieved and turned into a board that is hold by
		 * currentGameLevel.
		 * 
		 * It is crucial to call loadLevel() before enabling the rendering
		 * thread and picking bitmaps, because we need to view the map
		 * dimensions to decide the scaling factor and the borders size.
		 */
		...
		if (isServer) {
			...
			Thread client = new Thread(new WDClientThread(ip, port));
			client.start();
		}
	}
	...
	public void levelParsedCallback() {
		...
		/*
		 * Now that the screen arrangement has been decided, it is safe to start
		 * drawing.
		 */
